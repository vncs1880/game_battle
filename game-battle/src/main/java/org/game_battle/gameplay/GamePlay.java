...
//connected graph map representing a world map, where 
//
//each node is a country and 
//each edge represents adjacency between countries. 
//
...
//Map: The game map is a connected graph where each node represents a country owned by one of the
//players. Edges between the nodes represent adjacency between countries. The map is divided into
//subgraphs that represent continents. A continent is a connected subgraph of the map graph, and every
//country belongs to one and only one continent. 
//
//Board HAS Continents
//
//Continent HAS Countries
//
//Each continent is given a control value that determines the number of armies per turn that is given to a player that controls all of it. 
//
//Continent IS N_ArmiesPerTurn (not important for now)
//
//During game play, every country belongs to one and only one player and contains one or more armies that belong to the player owning the country. 
//
//Country HAS armies
//
//In your implementation, it will be expected that the game can be played on any
//connected graph that is defined by the user before play, saved as a text file representation, and loaded
